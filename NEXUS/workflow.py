...
def manual_review_hold_node(state: StudioState):
    ...
    state.review_queue_entry = build_review_queue_entry_safe(
        ...
        governance_status=state.governance_status,
        project_lifecycle_status=state.project_lifecycle_status,
        mission_status=getattr(state, "mission_status", None),
        mission_id=getattr(state, "mission_id", None),
        mission_title=getattr(state, "mission_title", None),
        mission_risk_level=getattr(state, "mission_risk_level", None),
        mission_stop_condition_hit=getattr(state, "mission_stop_condition_hit", None),
        mission_stop_condition_reason=getattr(state, "mission_stop_condition_reason", None),
        mission_requires_initial_approval=getattr(state, "mission_requires_initial_approval", None),
        mission_requires_final_approval=getattr(state, "mission_requires_final_approval", None),
    )
    return state
...
def save_persistent_project_state_node(state: StudioState):
    ...
    try:
        ...
        saved_path = save_project_state(
            ...
            autopilot_status=getattr(state, "autopilot_status", None),
            autopilot_loop_state=getattr(state, "autopilot_loop_state", None),
            autopilot_last_run_at=getattr(state, "autopilot_last_run_at", None),
            autopilot_next_run_at=getattr(state, "autopilot_next_run_at", None),
            autopilot_current_focus=getattr(state, "autopilot_current_focus", None),
            autopilot_requires_operator_review=getattr(state, "autopilot_requires_operator_review", None),
            mission_packet=getattr(state, "mission_packet", None),
            mission_id=getattr(state, "mission_id", None),
            mission_type=getattr(state, "mission_type", None),
            mission_title=getattr(state, "mission_title", None),
            mission_objective=getattr(state, "mission_objective", None),
            mission_scope_boundary=getattr(state, "mission_scope_boundary", None),
            mission_allowed_actions=getattr(state, "mission_allowed_actions", None),
            mission_forbidden_actions=getattr(state, "mission_forbidden_actions", None),
            mission_allowed_executors=getattr(state, "mission_allowed_executors", None),
            mission_risk_level=getattr(state, "mission_risk_level", None),
            mission_success_criteria=getattr(state, "mission_success_criteria", None),
            mission_stop_conditions=getattr(state, "mission_stop_conditions", None),
            mission_status=getattr(state, "mission_status", None),
            mission_created_at=getattr(state, "mission_created_at", None),
            mission_started_at=getattr(state, "mission_started_at", None),
            mission_completed_at=getattr(state, "mission_completed_at", None),
            mission_failed_at=getattr(state, "mission_failed_at", None),
            mission_requires_initial_approval=getattr(state, "mission_requires_initial_approval", None),
            mission_requires_final_approval=getattr(state, "mission_requires_final_approval", None),
            mission_stop_condition_hit=getattr(state, "mission_stop_condition_hit", None),
            mission_stop_condition_reason=getattr(state, "mission_stop_condition_reason", None),
            mission_escalation_required=getattr(state, "mission_escalation_required", None),
            executor_route=getattr(state, "executor_route", None),
            executor_route_reason=getattr(state, "executor_route_reason", None),
            executor_route_confidence=getattr(state, "executor_route_confidence", None),
            executor_route_status=getattr(state, "executor_route_status", None),
            executor_task_type=getattr(state, "executor_task_type", None),
            executor_task_packet=getattr(state, "executor_task_packet", None),
            executor_fallback_route=getattr(state, "executor_fallback_route", None),
