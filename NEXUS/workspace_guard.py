...
from NEXUS.studio_config import STUDIO_ROOT
from NEXUS.workspace_layers import LAYER_REGISTRY
...
def classify_path(path: str | Path, studio_root: str | Path | None = None) -> dict[str, Any]:
    ...
    first = parts[0].lower()
    if first == "nexus":
        return _summary(input_str, str(normalized), "NEXUS", root)
    if first == "core":
        return _summary(input_str, str(normalized), "core", root)  # legacy
    if first == "shared":
...
def inspect_project_paths(project_path: str | None, studio_root: str | Path | None = None) -> list[dict[str, Any]]:
    ...
    Includes: NEXUS/ (or legacy core/), project path, project/generated,
    project/state, project/docs, project/memory, project/tasks.
    """
    root = Path(studio_root or STUDIO_ROOT).resolve()
    results: list[dict[str, Any]] = []

    nexus_dir = root / "NEXUS"
    results.append(classify_path(nexus_dir, studio_root))
